...
static void test_failed_write_A_preserves_old_B(void) {
    ...
    f.fail_erase_a = true;
    assert(FlashStore_Save(&s, newer, sizeof(newer)) == FLASH_STORE_ERROR_WRITE);
    assert(FlashStore_Load(&s, out, sizeof(out)) == FLASH_STORE_OK);
...
static void test_A_newer_CRC_diff_repairs_B(void) {
    ...
    assert(FlashStore_Save(&s, v1, sizeof(v1)) == FLASH_STORE_OK);

    /* second save — write A (v2), then B erase fails → B still v1 */
    f.fail_erase_b = true;
    assert(FlashStore_Save(&s, v2, sizeof(v2)) == FLASH_STORE_ERROR_WRITE);
...
static void test_both_pages_corrupt_returns_error(void) {
    ...
    memset(f.page_b, 0, 4);
    assert(FlashStore_Load(&s, out, sizeof(out)) == FLASH_STORE_ERROR_NO_VALID_DATA);
}
...
static void test_max_data_size(void) {
    ...
    size_t max = FlashStore_MaxDataSize(&s);
...
static void test_init_rejects_bad_args(void) {
    ...
    FlashStore_Config cfg = {
        .io = {.read = fake_read, .erase = fake_erase, .program = fake_program},
        .page_a_address = PAGE_A_ADDRESS,
        .page_b_address = PAGE_B_ADDRESS,
        .page_size = PAGE_SIZE,
    };

    assert(FlashStore_Init(NULL, &cfg) == FLASH_STORE_ERROR_ARGUMENT);
    assert(FlashStore_Init(&store, NULL) == FLASH_STORE_ERROR_ARGUMENT);
    cfg.io.read = NULL;
    assert(FlashStore_Init(&store, &cfg) == FLASH_STORE_ERROR_ARGUMENT);
    cfg.io.read = fake_read;
    cfg.io.erase = NULL;
    assert(FlashStore_Init(&store, &cfg) == FLASH_STORE_ERROR_ARGUMENT);
    cfg.io.erase = fake_erase;
...
static void test_warn_repair_failed(void) {
    ...
    FlashStore_Status status = FlashStore_Load(&s, out, sizeof(out));
