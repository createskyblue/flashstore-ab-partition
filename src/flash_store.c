...
FlashStore_Status FlashStore_Load(FlashStore *store,
    ...
    bool a_ok = load_page(store, a, data, size, &crc_a);
    bool b_ok = load_page(store, b, data, size, &crc_b);
    ...
    if (a_ok && b_ok) {
        if (crc_a != crc_b) {
             ...
             * repair B.
             */
            load_page(store, a, data, size, NULL);
            FlashStore_Status r = save_page(store, b, data, size);
    ...
    if (a_ok) {
        load_page(store, a, data, size, NULL);
        repair = save_page(store, b, data, size);
    } else {
        repair = save_page(store, a, data, size);
    }
